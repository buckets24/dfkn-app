...
import { ClientControls } from 'src/views/client/controls/ClientControls';
import { AuthGuard } from 'src/views/common/AuthGuard';
import { gotoClientHome } from 'src/views/common/routing';
import { DocumentRenderer } from 'src/views/video-conference/DocumentRenderer';

/**
 * Memoized component
 */
const ClientEditSingleDocument: FC = memo(() => {
  const me = useAuthStore(getMe);
  const router = useRouter();
  const activeDocumentId = routerQueryGetAsString(router.query.documentId);
  const [fullscreen, setFullscreen] = useState<boolean>(false);
  if (activeDocumentId && typeof activeDocumentId !== 'string') {
    throw new Error(`Can't render ClientEditSingleDocument since activeDocumentId is not a string`);
  }
  ...
  useDocumentsByClientIdQuery(me?.id, {
    onError: (e: any) => {
      if (isUnauthorizedError(e)) {
        gotoClientHome(router);
      }
...
export default ClientEditSingleDocument;
