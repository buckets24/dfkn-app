...
import { ClientActivityTable } from 'src/modules/activity/table/ClientActivityTable';
...
const ClientActivity: FC & HasLayout = () => {
  const router = useRouter();
  const clientId = router.query.id;
  const toast = useToast();

  if (typeof clientId !== 'string') {
    throw new Error(`Can't render ClientActivity since clientId is not a string`);
  }
  ...
  useClientByIdQuery(clientId, {
    ...
    onError: (e: any) => {
      ...
      const errorCode = log(LogLevel.error, e?.message ?? 'useClientByIdQuery() in ClientActivity', e);
...
ClientActivity.getLayout = getAgentClientLayout;
